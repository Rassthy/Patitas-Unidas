      ...
      <div id="tab-valoraciones" class="profile-tab-content">
        ...
        <div class="review-form-container">
          ...
          <form action="{{ route('profile.rate', $user->id) }}" method="POST" id="rateForm">
            ...
            <div class="star-rating" id="starRating" style="font-size: 2rem; cursor: pointer; display: inline-flex; gap: 5px;">
            ...
            <textarea name="comentario" placeholder="Deja un comentario (opcional)..." class="edit-form-textarea" style="min-height: 60px; margin-bottom: 10px;"></textarea>

            <button class="btn-p btn-s" type="submit">Enviar valoración</button>
          </form>
        </div>

        <!-- Notificación centrada de sobrescritura (Oculta por defecto) -->
        <div id="overwriteWarning" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); background:white; padding:24px; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.3); z-index:9999; text-align:center; max-width: 400px;">
          <div style="font-size: 2rem; color: var(--terra); margin-bottom: 10px;"><i class="fa-solid fa-triangle-exclamation"></i></div>
          <h3 style="margin-top:0;">Actualizar valoración</h3>
          <p>Ya tienes una valoración en este perfil. Si decides enviar esta, la antigua se borrará y se sustituirá por la nueva.</p>
          <div style="display: flex; gap: 10px; justify-content: center; margin-top: 20px;">
            <button type="button" class="btn-s" onclick="document.getElementById('overwriteWarning').style.display='none'">Cancelar</button>
            <button type="button" class="btn-p" onclick="submitRatingAnyway()">Sí, reemplazar</button>
          </div>
        </div>
        @endif
        @endauth
        ...
        <div class="review-item">
          <div class="review-header">
            ...
            <span class="review-stars">
              <!-- Lógica de Blade para pintar estrellas completas o medias -->
              @php
    ...
    <div class="profile-card">
      <div class="edit-section-title" style="margin-bottom: 24px;">
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
          <span><i class="fa-solid fa-paw" style="color:var(--terra)"></i> Publicaciones</span>
          <span class="badge" style="font-size: 0.8rem; background: var(--terra); color: white; padding: 4px 12px; border-radius: 20px;">
            {{ $user->posts->count() }}
          </span>
        </div>
      </div>
      ...
      <div class="posts-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px;">
        @forelse($user->posts as $post)
          <a href="{{ route('posts.show', $post->id) }}" class="post-card-link" style="text-decoration: none; color: inherit;">
            <div class="post-card" style="border: 1px solid #eee; border-radius: 12px; overflow: hidden; background: white; transition: transform 0.2s; height: 100%;">
              @if($post->imagen_url)
              ...
              <div style="padding: 15px;">
                ...
                <p style="font-size: 0.9rem; color: #666;">{{ Str::limit($post->contenido, 80) }}</p>
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 10px; padding-top: 10px; border-top: 1px solid #f5f5f5;">
                  ...
                  <span style="color: var(--terra); font-weight: 600; font-size: 0.85rem;">Leer más →</span>
                </div>
              </div>
            </div>
          </a>
        @empty
          <div class="empty-state" style="grid-column: 1 / -1;">
            ...
            <p class="empty-state-desc">Cuando este usuario publique algo, aparecerá aquí.</p>
          </div>
        @endforelse
      </div>
    </div>
      ...
      rateForm.addEventListener('submit', function (e) {
        ...
        if (!selectedValue) {
          if (typeof showToast === 'function') {
            showToast('Debes marcar al menos media estrella para valorar.', 'error');
          }
          return;
        }
